...
function tambahproduk(qtyAdd){
	...
	if ($('#hiddenbarcode').val() != '' && $('#hiddenbarcode').val().length > 5){
    ...
    var xqty = 0;
	if(katacari.startsWith(CUSTOMCODE)){
		xqty = katacari.substr(7, 5)/100;
		console.log(xqty);
		katacari = katacari.substr(0, 7);		 
	}
...
function akhiri_belanja(cetak){
	...
	if (Math.round(kembalian*100/100) >= 0){
...
function hitung_omset(){
	...
	$.ajax({
		...
		success: function(data){
			...
			$("#pesantext").text("TOTAL JUALAN HARI INI ["+ tgltampil +"] : "+ currSym +" "+ omsetsekarang);
			$("#dialogwarning").dialog("open");
		}
	});
}
...
function tambah_barang(){
	$("#dialogtambahbarang").dialog("open");
}
...
$(document).ready(function(){
    ...
    $("#newharga1").keyup(function(e){
		if(e.keyCode == 13){
			$("#buttonsubmitbarang").click();		
		}
	});
	$("#buttonsubmitbarang").click(function(){
		var formValues = {barcode : $("#newbarcode1").val(),nama:$("#newnama1").val(),harga:$("#newharga1").val()};
		alamat = pathutama + "penjualan/tambahbarang";
		$.ajax({
			type: "POST",
			url: alamat,
			cache: false,
			data : formValues,
			success: function (data) {
				$("#dialogtambahbarang").dialog("close");
			}
		});
	...
	$("#nilaibayar,#nomerkartu").keyup(function(e){
        var total_plus_ppn =  totalbelanja;
		kembali = $("#nilaibayar").val() - total_plus_ppn;
        var kembaliView = parseFloat(kembali).toFixed(2);
		$("#kembali").val(currSym +" "+ addCommas(kembaliView));
		if (kembali > 0){
