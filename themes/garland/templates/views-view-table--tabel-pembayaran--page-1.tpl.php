 ...
 */
 ...
 drupal_add_js('
  ...
  $(document).ready(function(){
	 	oTable = $("#tabel_pembayaran_gaji").dataTable( {
			"bJQueryUI": true,
			"bPaginate": true,
			"bLengthChange": true,
			"bFilter": true,
 ...
 ','inline');
	...
	$superuser = false;
 	...
 	if ($superuser){
 		$alamat_tambah_pembayaran = base_path().'node/add/pembayaran-gaji';	
 	}else{
 		$view = views_get_view('get_posisi_user');
		$view->execute();
		if (count($view->result)){
			$id_posisi_user = $view->result[0]->node_data_field_user_id_field_cabang_id_nid;
			$alamat_tambah_pembayaran = base_path().'node/add/pembayaran-gaji/'.$id_posisi_user;
		}	
 	}
...
?>
<button onclick="goto_address('<?php print $alamat_tambah_pembayaran; ?>')" class="add_data">TAMBAH PEMBAYARAN GAJI</button>
<table id="tabel_pembayaran_gaji" class="<?php print $class; ?>">
  <?php if (!empty($title)) : ?>
