 ...
 */
 ...
 drupal_add_js('
  ...
  $(document).ready(function(){
	 	oTable = $("#tabel_supir_indogas").dataTable( {
			"bJQueryUI": true,
			"bPaginate": true,
			"bLengthChange": true,
			"bFilter": true,
 ...
 ','inline');
	...
	$superuser = false;
 	...
 	if ($superuser){
 		$alamat_tambah_supir = base_path().'node/add/data-supir';	
 	}else{
 		$view = views_get_view('get_posisi_user');
		$view->execute();
		if (count($view->result)){
			$id_posisi_user = $view->result[0]->node_data_field_user_id_field_cabang_id_nid;
			$alamat_tambah_supir = base_path().'node/add/data-supir/'.$id_posisi_user;
		}	
 	}
...
?>
<button onclick="goto_address('<?php print $alamat_tambah_supir; ?>')" class="add_data">TAMBAH SUPIR</button>
<table id="tabel_supir_indogas" class="<?php print $class; ?>">
  <?php if (!empty($title)) : ?>
