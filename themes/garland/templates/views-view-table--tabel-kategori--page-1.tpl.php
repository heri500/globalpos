 ...
 */
 ...
 drupal_add_js('
  ...
  $(document).ready(function(){
	 	oTable = $("#tabel_kategori_indogas").dataTable( {
			"bJQueryUI": true,
			"bPaginate": true,
			"bLengthChange": true,
			"bFilter": true,
 ...
 ','inline');
	...
	$superuser = false;
 	...
 	if ($superuser){
 		$alamat_tambah_kategori = base_path().'node/add/kategori-material';	
 	}
 	if ((arg(2) <> $id_posisi_user) AND !$superuser){
 		drupal_goto('inventory');
 	}
?>
<button onclick="goto_address('<?php print $alamat_tambah_kategori; ?>')" class="add_data">TAMBAH KATEGORI</button>
<table id="tabel_kategori_indogas" class="<?php print $class; ?>">
  <?php if (!empty($title)) : ?>
