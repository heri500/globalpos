  ...
  <thead>
    <tr>
      <?php foreach ($header as $field => $label): ?>
        <?php
	      if ($field != 'field_nilai_transaksi_value' && $field != 'field_posisi_account_jurnal_value' && $field != 'status' && $field != 'nid' && $field != 'nid_1'){
	      ?>
    ...
    <?php foreach ($rows as $count => $row): ?>
      <tr class="<?php print implode(' ', $row_classes[$count]); ?>">
        ...
        <?php
        	...
        	$nid_jurnal_ref = $rows[$count]['nid_1'];
        	...
        	if ($posted_count > 0){
        		...
        		if ($superuser){ 
        			$statusview = '<img title="Jurnal Belum diposting, klik untuk mengedit jurnal ini" onclick="edit_jurnal(\''.$nid_jurnal_ref.'\')" src="'.base_path().'misc/media/images/warning.png" style="cursor: pointer;">';	
        		}else{
        			$statusview = '<img title="Jurnal Belum diposting" src="'.base_path().'misc/media/images/warning.png" style="cursor: pointer;">';	
        		}
        	}
        ?>
