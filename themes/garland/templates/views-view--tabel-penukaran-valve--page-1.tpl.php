  ...
  $datatables = true;
	...
	drupal_add_js('
		...
		function edit_penukaran(nid){
			window.location = Drupal.settings.basePath + "node/"+ nid +"/edit?destination=produksi/tabelpenukaranvalve";
		}
		function delete_penukaran(nid, nTr){
			var konfirmasi = confirm("Yakin akan menghapus penukaran valve ini...??!");
			if (konfirmasi){
				$("div.view-id-tabel_penukaran_valve").block({ message: \'<p style=\"color: 808080;padding: .2em;font-size: 18px;\">Deleting...<img src=\"../misc/media/images/loading.gif\"></p>\',css: { border: \'3px solid #a00\' } });
				var posisibaris = oTable.fnGetPosition(nTr);
				var request = new Object;
				request.nid = nid;
				submitaddress = Drupal.settings.basePath + "hapusnodebynid";
				$.ajax({ 
					type: "POST",
					url: submitaddress,
					data: request, 
					...
					success: function(data){
						alert(data);
						oTable.fnDeleteRow(posisibaris);
						$("div.view-id-tabel_penukaran_valve").unblock();
					}
				});
			}	
		}
		$(document).ready(function(){
			oTable = $("#tabel_penukaran_valve").dataTable( {
				"bJQueryUI": true,
				"bPaginate": false,
				"bLengthChange": false,
				"bFilter": true,
	   	...
	   	$("#edit-created-max-wrapper label").css("margin-right","8px");
			$("#edit-submit-tabel-penukaran-valve").text("Filter Data");
			$("#edit-submit-tabel-penukaran-valve").css("font-size","12px");
			$("#edit-submit-tabel-penukaran-valve").css("padding","0.3em 1em");
			$("#edit-submit-tabel-penukaran-valve").button();
			$("#edit-submit-tabel-penukaran-valve").parent().css("width","auto");
			$("#edit-submit-tabel-penukaran-valve").css("margin-top","0");
			...
			$("#views-exposed-form-tabel-penukaran-valve-page-1").prepend($("#pilihancabang"));
			$("#views-exposed-form-tabel-penukaran-valve-page-1").css("margin-bottom","0");
			$("#views-exposed-form-tabel-penukaran-valve-page-1").parent().attr("id","form_jurnal_umum");
			if (superuser){
	...
	if ($superuser){
 	...
 	if ($superuser){
 		$alamat_tambah_penukaran = base_path().'node/add/penukaran-valve';	
		...
		if (count($view->result)){
			...
			$alamat_tambah_penukaran = base_path().'node/add/penukaran-valve/'.$id_posisi_user;
		}	
 	}
?>
