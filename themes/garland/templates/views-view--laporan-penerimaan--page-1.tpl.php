 ...
 */
 ...
 drupal_add_js('
  ...
  function delete_penerimaan(nid_penerimaan, nTr){
  	var konfirmasi = confirm("Apakah penerimaan ini benar-benar ingin dihapus...??!");
  	if (konfirmasi){
	  	var request = new Object;
			request.nid_penerimaan = nid_penerimaan;
			submitaddress = mainpath + "deletepenerimaantabung";
			$.ajax({ 
				type: "POST",
				url: submitaddress,
				data: request, 
  ...
  $(document).ready(function(){
   	...
   	$("#edit-created-max-wrapper label").css("margin-right","8px");
		...
		$("#views-exposed-form-laporan-penerimaan-page-1").prepend($("#pilihancabang"));
		$("#views-exposed-form-laporan-penerimaan-page-1").css("margin-bottom","0");
		$("button").button();
	})
 ','inline');
 global $user;
