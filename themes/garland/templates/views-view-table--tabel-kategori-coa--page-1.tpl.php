 ...
 */
 ...
 drupal_add_js('
  ...
  $(document).ready(function(){
	 	oTable = $("#tabel_kategori_coa").dataTable( {
			"bJQueryUI": true,
			"bPaginate": true,
			"bLengthChange": true,
			"bFilter": true,
 ...
 ','inline');
	...
	$superuser = false;
 	...
 	if ($superuser){
 		$alamat_tambah_kategori = base_path().'node/add/coa-category';	
 	}
 	if ((arg(2) <> $id_posisi_user) AND !$superuser){
 		drupal_goto('akutansi');
 	}
?>
<button onclick="goto_address('<?php print $alamat_tambah_kategori; ?>')" class="add_data">TAMBAH KATEGORI COA</button>
<table id="tabel_kategori_coa" class="<?php print $class; ?>">
  <?php if (!empty($title)) : ?>
