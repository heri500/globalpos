 ...
 */
 ...
 drupal_add_js('
  ...
  $(document).ready(function(){
	 	oTable = $("#tabel_subkategori_coa").dataTable( {
			"bJQueryUI": true,
			"bPaginate": true,
			"bLengthChange": true,
			"bFilter": true,
 ...
 ','inline');
	...
	$superuser = false;
 	...
 	if ($superuser){
 		$alamat_tambah_subkategori = base_path().'node/add/coa-category';	
 	}
 	if ((arg(2) <> $id_posisi_user) AND !$superuser){
 		drupal_goto('akutansi');
 	}
?>
<button onclick="goto_address('<?php print $alamat_tambah_subkategori; ?>')" class="add_data">TAMBAH SUBKATEGORI COA</button>
<table id="tabel_subkategori_coa" class="<?php print $class; ?>">
  <?php if (!empty($title)) : ?>
  ...
  <thead>
    <tr>
      ...
      <?php foreach ($header as $field => $label): ?>
      	...
      	<?php
      		if ($field == 'title' || $field == 'field_kategori_coa_ref_nid'){
      			$lebar = 'style="width: 100px;"';	
      		}else{
      			$lebar = '';
      		}
      	?>
